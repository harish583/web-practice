...
//     answers.push(questions[i].options);
// }
// var quiz = [
//     {
//         id: 'question1',
...
for(var i=0;i<quats.length;i++){
    ...
    answers.push(quats[i].correct-1);
}
        rendering(quats);
      }
    };
...
function forNextQuation(id,quiz){
    ...
    if(id==quiz.length-1){
        ...
        submitting.addEventListener("click",function(){
            collectingSelectedValues(quiz);
        });

    }
    
}
function collectingSelectedValues(quiz){
    var selectedOptions = [];
    for(var i=0;i<quiz.length;i++){
    ...
    showingAnswers(selectedOptions,quiz);
}
var showingAnswer1='';
var finale='';
var finle ='';
var html7 ='';
function showingAnswers(selectedOptions,quiz){
