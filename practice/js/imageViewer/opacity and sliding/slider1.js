...
//binding events
function bindind(flower1,format){
    ...
    for(var i =0;i<flower1.length;i++){
        ...
        if(format == "slide"){div1.classList.add("opacity1")};
    ...
    if(format == "opacity"){flower[index].classList.add("opacity");}
//for arrow binding
...
//for checkboxes binding
    ...
    if(format == "slide"){
        var forMargin = document.querySelector(".flower1");
...
function checkBox(value){
    ...
    if(format == "slide"){
        var flower = document.querySelector(".flower1");
        flower.style.marginLeft = -(value * 100) + '%';
    }
}
...
//  function bindind(flower1,format);
bindind(flowers2,"opacity");
// bindind(flowers2,"slide");
